<template>
    <div style="width:100%;min-height: calc(100vh - 84px);background-color:#EEEEEE">
            ...
            <el-main width="78%" style="padding:0" class="right-box">
                ...
                <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch"
                    ...
                    <el-form-item label="日期" prop="dateTime">
                        <el-date-picker v-model="queryParams.dateTime" type="datetime" placeholder="选择一个日期" />
                    </el-form-item>
                    <el-form-item>
                ...
                <el-table v-loading="tableLoading" :data="fileList" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column label="是否公开" align="center" prop="fileStatus" v-hasPermi="['system:file:edit']">
                    ...
                    <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                        <template #default="scope">
                            <el-button size="small" type="text" icon="Download" @click="handleDownload(scope.row)">下载
                            </el-button>
                            <el-button size="small" type="text" icon="Document" @click="openDrawer(scope.row)">预览
                            </el-button>
        ...
        <!-- 节点对话框 -->
        <el-dialog :title="textMap[dialogTreeStatus]" v-model="dialogTreeFormVisible" center draggable width="50%">
            <el-form ref="dataTreeForm" :model="treeForm" :rules="treeRules" label-position="left" label-width="100px">
                <el-form-item label="节点名称：" prop="treeName">
                    <el-input v-model="treeForm.treeName" placeholder="输入节点名称" />
                </el-form-item>
            ...
            <template #footer>
                <div class="dialog-footer">
                    <el-button type="primary"
                        @click.passive="dialogTreeStatus === 'createNode' ? createTreeData() : updateTreeData()">
                        保存
                    </el-button>
        ...
        <el-dialog :title="textMap[dialogStatus]" v-model="dialogFormVisible" center draggable width="50%">
            <el-form ref="form" :rules="rules" :model="dataForm" label-position="left" label-width="100px">
                ...
                <el-form-item label="日期" prop="dateTime">
                    <el-date-picker v-model="dataForm.dateTime" type="datetime" placeholder="选择一个日期" />
                </el-form-item>
...
function createData() {
    form.value.validate(valid => {
        if (valid) {
            uploadUrl.value =
                `${import.meta.env.VITE_APP_UPLOAD_URL}/system/file/upload?fileName=${dataForm.fileName}&description=${dataForm.description}&fileStatus=${dataForm.fileStatus ? 1 : 0}&treeId=${tree.value.getCurrentNode().treeId}&dateTime=${parseTime(dataForm.dateTime)}`;
            nextTick(async () => {
...
async function updateData() {
    form.value.validate(valid => {
        if (valid) {
            ...
            updateFile({ ...dataForm, dateTime: parseTime(dataForm.dateTime) }).then(res => {
                tableLoading.value = false;
                dialogFormVisible.value = false;
                getList();
            }).catch(err => {
...
function createTreeData() {
    dataTreeForm.value.validate(valid => {
        if (valid) {
            ...
            addNode({ isShow: treeForm.isShow ? 1 : 0, treeName: treeForm.treeName, parentId: id, treeType: treeType.value }).then(() => {
...
function updateTreeData() {
    dataTreeForm.value.validate(valid => {
        if (valid) {
            updateNode({ isShow: treeForm.isShow ? 1 : 0, treeName: treeForm.treeName, treeId: tree.value.getCurrentNode().treeId }).then(() => {
